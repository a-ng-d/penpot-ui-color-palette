...
const updateLocalVariables = async (palette: Board) => {
  /*const paletteData: PaletteData = JSON.parse(palette.getPluginData("data"));
  const canDeepSyncVariables = await window.localStorage.getItem(
      ...
      const updateLocalVariablesStatusMessage = figma.variables
        ...
        .then((localVariables) => {
          ...
          if (i > 1 && j > 1)
            ...
            messages.push(
              locals[lang].info.updatedVariablesAndModes.singleNone
            );
          else if (i === 0 && j === 1)
            messages.push(
              locals[lang].info.updatedVariablesAndModes.noneSingle
            );
          ...
          if (l > 1 && m > 1)
            ...
            messages.push(
              locals[lang].info.removedVariablesAndModes.singleNone
            );
          else if (l === 0 && m === 1)
            messages.push(
              locals[lang].info.removedVariablesAndModes.noneSingle
            );
