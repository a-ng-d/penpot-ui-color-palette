...
const createLocalVariables = async (palette: Array<Shape>) => {
  /*palette = figma.currentPage.selection[0] as Board;
    ...
    const createLocalVariablesStatusMessage = figma.variables
      ...
      .then((localVariables) => {
        ...
        themesList.forEach((themeItem) => {
          if (collection !== undefined && themeItem !== undefined) {
            if (createdVariables.length === 0)
              createdVariables = localVariables;
        ...
        if (i > 1 && j > 1)
          ...
          messages.push(
            locals[lang].info.createdVariablesAndModes.singleSingle
          );
        else if (i === 0 && j === 0)
